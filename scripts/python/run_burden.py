...
def run_burden(config: dict[str, Any], project_root: Path) -> Path:
    ...
    result_tsv = output_dir / "burden_result.tsv"
    score_tsv = output_dir / "burden_scores.tsv"
    ...
    if engine == "regenie":
        ...
        covariates = config.get("burden", {}).get("covariates", [])
        regenie_bin = config.get("burden", {}).get("regenie_bin", "regenie")
        aaf_bins = str(config.get("burden", {}).get("regenie_aaf_bins", "0.01"))
        build_mask = str(config.get("burden", {}).get("regenie_build_mask", "max"))
        ignore_pred = bool(config.get("burden", {}).get("regenie_ignore_pred", False))
        pred_value = config.get("input", {}).get("pred_file")
        pred_file = project_root / pred_value if pred_value else None
        bsize = int(config.get("burden", {}).get("regenie_bsize", 200))
        extra_args = config.get("burden", {}).get("regenie_extra_args", [])
        regenie_prefix = output_dir / "regenie_burden"
        ...
        if pred_file is not None:
            ...
            raise FileNotFoundError(
                ...
                "burden.regenie_ignore_pred=true for a small demo run."
            )
...
def main() -> int:
    raise SystemExit(
        "This module is intended to be imported by main.py, not run directly."
    )
